...
3.  **Show** the table.
    ...
    1.  **Show** the `data.frame` in the R console. Convert the `data.frame` to a tidyverse `tibble()` and show it again, now with a more informative summary.

    2.  **Render** the table into HTML with `DT::datatable()`.

4.  **Wrangle** with `dplyr`, `tidyr` and `purrr`.
    ...
    5.  **Nest** tables inside of table cells. Use this to build a model object into another cell using `purrr::map()`.
...
Please visit the ERDDAP server [oceanview.pfeg.noaa.gov/erddap](oceanview.pfeg.noaa.gov/erddap) and please do a *Full Text Search for Datasets* using "cciea" in the text box before clicking **Search**. These are all the California Current IEA datasets.
