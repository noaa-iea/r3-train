...
1.  **Wrangle** with `tibble`, `dplyr`, `tidyr` and `purrr`.
    ...
    5.  **Nest** tables inside of tables with a `tibble` and tidyr's `nest()`. Build a linear model object (`lm()`) into another **list column** per grouping using `purrr::map()`. Extract values from this model object as double with `purrr::map_dbl()`.
